...
class DistributionTable:
    ...
    def write_overall(self, label_set, overall):
        """
        Write overall information to a excel file
        """

        if overall is not None:
            sheet_oa = self.work_book.create_sheet('Overall')
    ...
    def write_items(self, label_set, items):
        """
        Write items to a excel file
        """
        if items is not None:
            sheet_items = self.work_book.create_sheet('Items')
            for c_idx, label in enumerate(label_set):
