...
def split_dataset_with_ratio(src_folder: str, dst_folder:str, num_pieces: int, kept_pieces: int, \
    ...
    for phase in phases:
        ...
        for p_idx in range(num_pieces):
            ...
            s_pieces = p_idx if phase in scopes else 0
            for s_idx in range(s_pieces, len(samples), p_pieces):
                shutil.copy(os.path.join(src_phase_path, samples[s_idx]), \
